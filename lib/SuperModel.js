          ...
          return function(robj) {
            return (function(resolveobj) {
              ...
              if (!resolveobj.ids) {
                ...
                if (debug) {
                  console.log(' resolveobjds ' + resolveobj.type + ' (' + (typeof resolveobj.ids) + ') ids length are.. ' + resolveobj.ids.length);
                }
                if (debug) {
                ...
                if (count === 0) {
                  if (debug) {
                    console.log('no ids for ' + resolveobj.type + ' so resolving null');
                  }
                  return r.resolve(null);
                } else {
                  return resolveobj.ids.forEach(function(id) {
                    ...
                    return OMgr.getObject(id, resolveobj.type).then(function(oo) {
                      if (oo) {
                        if (debug) {
                          console.log('found existing instance of ' + resolveobj.type + ' in OStore');
                        }
                        _this.insertObj(resolveobj, oo);
                        if (--count === 0) {
                          if (debug) {
                            console.log('resolving ' + resolveobj.name + ' immediately');
                          }
                          return r.resolve(oo);
                        }
                      } else {
