...
(function() {
  ...
  SuperModel = (function() {
    ...
    function SuperModel(record) {
      ...
      this.createObjectFromRecord = __bind(this.createObjectFromRecord, this);
      ...
      model.forEach((function(_this) {
        return function(v) {
          ...
          if ((v.value && v.value !== 0) && v.type) {
            if (debug) {
              console.log('getRecord accessing property ' + k + ' of object ' + _this.type + ' -> ' + me[k]);
            }
            if (me[k]) {
              if (v.storedirectly) {
      ...
      return OMgr.getObject(id, resolveobj.type).then((function(_this) {
        return function(oo) {
          if (oo) {
            ...
            if (debug) {
              console.log('did not find obj ' + resolveobj.name + ' [' + id + '] of type ' + resolveobj.type + ' in OStore. Getting from DB. typeof of id prop is ' + (typeof id));
            }
            return DB.get(resolveobj.type, [id]).then(function(record) {
              if (!record) {
                ...
                if (debug) {
                  console.log('** resolveObj no obj found and no record for id ' + id);
                }
