...
(function() {
  var Couch, DB, LRU, Mongo, OStore, ResolveModule, all, debug, defer, resolver, uuid;
  ...
  DB = (function() {
    ...
    DB.count = function(type) {
      ...
      q = defer();
      store = DB.getDataStore();
      store.count(type).then(function(value) {
    ...
    DB.find = function(type, property, value) {
      var q;
      q = defer();
      DB.getDataStore().find(type, property, value).then(function(result) {
        if (!result) {
    ...
    DB.findMany = function(type, property, value) {
      var q;
      q = defer();
      DB.getDataStore().findMany(type, property, value).then(function(results) {
        if (debug) {
    ...
    DB.findQuery = function(type, query) {
      var q;
      q = defer();
      DB.getDataStore().findQuery(type, query).then(function(results) {
        console.log('DB.findQuery type ' + type + ', property ' + property + ', value ' + value + ' got back ' + (results != null ? results.length : void 0) + ' results');
        if (results) {
          results.forEach(function(result) {
            return DB.lru.set(result.id, result);
          });
    ...
    DB.search = function(type, property, value) {
      var q;
      q = defer();
      DB.getDataStore().search(type, property, value).then(function(results) {
