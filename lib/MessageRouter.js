...
(function() {
  ...
  MessageRouter = (function() {
    ...
    function MessageRouter(authMgr, dburl, msgPS) {
      ...
      this.targets = [];
      this.messagesPerSecond = msgPS || 10;
      this.debugtargets = [];
      this.args = [];
      this.methods = [];
    ...
    MessageRouter.prototype.routeMessage = function(message) {
      ...
      if (fn) {
        return this.authMgr.decorateMessageWithUser(message).then(function(m) {
          ...
          if (!m.limiter) {
            m.limiter = new RateLimiter(this.messagesPerSecond, 1000);
          }
          return m.limiter.removeTokens(1, (function(_this) {
            return function(err, remainingRequests) {
              if (err) {
                return m.replyFunc({
                  status: e.general.NOT_ALLOWED,
                  info: 'packets over ' + _this.messagesPerSecond + ' dropped. Have a nice day.',
                  payload: {
