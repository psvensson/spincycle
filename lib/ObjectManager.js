    ...
    ObjectManager.prototype._getObject = function(msg) {
      ...
      if (msg.type && msg.obj && msg.obj.id) {
        ...
        if (id.indexOf && id.indexOf('all_') > -1) {
          ...
          if (debug) {
            console.log('_getObject calling getObjectPullThrough for type ' + msg.type);
          }
            ...
            return function(obj) {
              ...
              if (obj) {
                if (_this.messageRouter.authMgr.canUserReadFromThisObject(obj, msg.user)) {
                  tc = obj.toClient();
                  if (debug) {
                    console.log('_getObject for ' + msg.type + ' returns');
                  }
                  if (debug) {
                    console.dir(tc);
                  }
        ...
        return _records.forEach((function(_this) {
          return function(r) {
            return DB.get(r.type, [r.id]).then(function(records) {
              if (debug) {
                console.log('ObjectManager.parseList -- result of getting record ' + r.type + ' id ' + r.id + ' is ' + records);
              }
              if (debug) {
                console.dir(records);
              }
        ...
        objStore.getObject(id, type).then((function(_this) {
          return function(o) {
            if (!o) {
              ...
              if (debug) {
                console.log('- getObjectPullThrough found object in objStore');
              }
              return q.resolve(o);
            }
          };
