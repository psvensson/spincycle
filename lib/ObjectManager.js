...
(function() {
  ...
  ObjectManager = (function() {
          ...
          return function(r) {
            return DB.get(r.type, [r.id]).then(function(record) {
              ...
              if (record && record[0]) {
                ...
                tc = record[0];
                if (_this.messageRouter.authMgr.filterOutgoing) {
                  _this.messageRouter.authMgr.filterOutgoing(tc, msg.user).then(function(tres) {
                    if (debug) {
                      console.log('parseList got reply from filterOutgoing and it was');
                    }
                    if (debug) {
                      console.dir(tres);
                    }
              ...
              if (canwrite) {
                if (!_this.areDataTrashed(obj)) {
                  ...
                  return _this.resolveReferences(obj, obj.constructor.model).then(function(robj) {
                    return _this.persistUpdates(obj, robj).then(function(res) {
                      if (!res) {
                        return msg.replyFunc({
                          status: e.general.FAILURE,
    ...
    ObjectManager.prototype.persistUpdates = function(obj, robj, force) {
      ...
      obj.serialize(robj).then((function(_this) {
        return function(res) {
          var record;
          if (debug) {
            console.log('persisting returned from serialize');
          }
          if (!res) {
            if (debug) {
