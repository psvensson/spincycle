// Generated by CoffeeScript 1.9.1
(function() {
  var RedisMethod, redis, uuid,
    bind = function(fn, me){ return function(){ return fn.apply(me, arguments); }; };

  uuid = require('node-uuid');

  redis = require('redis');

  RedisMethod = (function() {
    RedisMethod.redisroutes = [];

    RedisMethod.client = redis.createClient();

    function RedisMethod(messageRouter, app, basePath) {
      this.expose = bind(this.expose, this);
      this.client.subscribe('spinchannel');
      this.client.on('message', RedisMethod.onChannelMessage);
      messageRouter.addMethod('redis', this);
    }

    RedisMethod.prototype.onChannelMessage = function(channel, message) {
      console.log('redismethod got channel ' + channel + ' message ' + message);
      console.dir(channel);
      console.lo('-------------------------------------------------------------------');
      return console.dir(messages);
    };

    RedisMethod.prototype.registrationFunc = function(targetName, targetFunc) {
      return RedisMethod.redisroutes[targetName] = targetFunc;
    };

    RedisMethod.prototype.expose = function(type) {
      return console.log('RedisMethod::Expose called for type ' + type + ' (unimplemented)');
    };

    return RedisMethod;

  })();

  module.exports = RedisMethod;

}).call(this);

//# sourceMappingURL=RedisMethod.js.map
