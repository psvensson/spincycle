...
(function() {
  ...
  MongoPersistence = (function() {
    ...
    function MongoPersistence(dburl1) {
      this.dburl = dburl1;
      ...
      this.setOplogCursor = bind(this.setOplogCursor, this);
    ...
    MongoPersistence.prototype.setOplogCursor = function(dburl) {
      return MongoClient.connect(dburl + '/local?authSource=spincycle', (function(_this) {
        return function(err, db) {
          return db.collection('oplog.rs', function(err, oplog) {
            return oplog.find({}, {
              ts: 1
            }).sort({
              $natural: -1
            }).limit(1).toArray(function(err, data) {
              var lastOplogTime, queryForTime, stream, tstamp;
              if (err) {
              ...
              if (data && data[0]) {
                ...
                return stream.on('data', function(oplogdoc) {
                  return console.log('OPLOG: --- ' + oplogdoc);
                });
              }
            });
          });
