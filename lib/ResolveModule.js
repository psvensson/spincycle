  ...
  ResolveModule = (function() {
    ...
    ResolveModule.prototype.createObjectFrom = function(record) {
      ...
      if (!record || (record[0] && (record[0] === null) || record[0] === 'null')) {
        ...
        if (debug) {
          console.log('ResolveModule.createObjectFrom resolving record with id ' + record[0].id + ' of type ' + record[0].type);
        }
        module = ResolveModule.modulecache[record[0].type];
        if (module) {
          if (debug) {
            console.log('createObjectFrom found existing module for ' + record[0].type);
          }
