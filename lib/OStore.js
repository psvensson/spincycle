...
(function() {
  ...
  OStore = (function() {
    ...
    OStore.updateObj = function(record, force) {
      ...
      if (obj) {
        ...
        for (p in whitelist) {
          for (pp in record) {
            if (pp === p) {
              if (__indexOf.call(OStore.blackList, pp) < 0) {
                if (debug) {
        ...
        if (OStore.anyoneIsListening(obj.id)) {
          if (!OStore.outstandingUpates[obj.id]) {
    ...
    OStore.sendUpdatesFor = function(obj, changed) {
      if (debug) {
        console.log('sendUpdatesFor called for obj ' + obj.id + ' changed = ' + changed + ', OStore.outstandingUpates[obj.id] = ' + OStore.outstandingUpates[obj.id] + ' anyone is listening == ' + OStore.anyoneIsListening(obj.id));
      }
      if (!OStore.outstandingUpates[obj.id] && changed && OStore.anyoneIsListening(obj.id)) {
        OStore.outstandingUpates[obj.id] = obj;
        if (debug) {
    ...
    OStore.anyoneIsListening = function(id) {
      return OStore.listeners[id] && OStore.listeners[id].length > 0;
    };
    ...
    OStore.sendAtInterval = function() {
      var count, diff, l, last, lid, listeners, obj;
      if (OStore.updateQueue.length > 0) {
        console.log('queue length = ' + OStore.updateQueue.length);
        l = OStore.updateQueue.length > 100 ? 100 : OStore.updateQueue.length;
        count = 0;
        while (count < l) {
          count++;
          obj = OStore.updateQueue.shift();
          last = obj._lastUpdate || 0;
          diff = Date.now() - last;
      ...
      return setTimeout(OStore.sendAtInterval, 100);
    };
